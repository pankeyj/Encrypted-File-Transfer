...
void recvMessage(Client *client)
{
	...
	printf("The encrypted key is %240s\n", recvBuffer);
	printf("The initialization vector is %16s\n", &recvBuffer[256]);
	sprintf(client->iv, "%16s", &recvBuffer[256]);
	FILE* privf = fopen("RSApriv.pem", "rb");
	privkey = PEM_read_PrivateKey(privf,NULL,NULL,NULL);
	unsigned char decrypted_key[256];
	int decryptedkey_len = rsa_decrypt(recvBuffer, 256, privkey,decrypted_key);

	printf("Length of decrypted key%d\n", decryptedkey_len);
	memcpy(client->symmetric_Key, decrypted_key, decryptedkey_len);
	printf("The decrypted random key %s%s\n", client->symmetric_Key, client->iv);	
	sprintf(decrypted_key, "%s", client->iv);
// 	i = recv(clientSocket, recvBuffer, 16, 0);
//	memcpy(client->iv, &recvBuffer[256], 16);
//	printf("This clients initialization vector %s\n", client->iv);
	...
	while(1)
	{
		...
		printf("Text Before Decryption %s\n", recvBuffer);
		int decryptedText_len = decrypt(recvBuffer, i, client->symmetric_Key, client->iv, decryptedText); 
		decryptedText[decryptedText_len] = '\0';
		printf("Length of decrypted Text %d\n", decryptedText_len);
		memset(recvBuffer, '\0', 1024);
		memcpy(recvBuffer, decryptedText, decryptedText_len);
